...
output_path = '';  % e.g., 'C:\YourResults\RF\'
...
% Optimized hyperparameter grid for 337 samples and 7 features
param_grid = struct();
...
total_combinations = length(param_grid.MinLeafSize) * ...
                     length(param_grid.NumPredictorsToSample) * ...
                     length(param_grid.NumTrees);
...
for min_leaf = param_grid.MinLeafSize
    for num_pred = param_grid.NumPredictorsToSample
        for num_trees = param_grid.NumTrees
            ...
            fprintf('Processing combination %d/%d (MinLeaf: %d, NumPred: %d, Trees: %d)\n', ...
            ...
            % Select best model (high R² with reasonable stability)
...
% If no model met stability criteria, select the one with highest R²
if best_r2_cv == -Inf
    ...
    fprintf('Selected best model based on R² (stability relaxed)\n');
end

%% 6. Train final model with OOB error calculation
...
final_model = TreeBagger(best_params.NumTrees, X_train_norm, y_train, ...
                        'Method', 'regression', ...
...
final_pred = predict(final_model, X_train_norm);

% Final evaluation metrics
final_r2 = corr(final_pred, y_train)^2;
final_rmse = sqrt(mean((final_pred - y_train).^2));
final_mae = mean(abs(final_pred - y_train));
residuals = final_pred - y_train;
...
% Additional metrics
nse = 1 - (sum((y_train - final_pred).^2) / sum((y_train - mean(y_train)).^2));
pbias = 100 * (sum(y_train - final_pred) / sum(y_train));
generalization_gap_r2 = final_r2 - best_validation_metrics.Mean_R2;

% Variable importance (relative for comparison with ANN)
raw_importance = final_model.OOBPermutedPredictorDeltaError;
relative_importance_rf = raw_importance / sum(raw_importance);
...
subplot(2,3,3);
bar(relative_importance_rf, 'FaceColor', [0.8 0.4 0.2]);
...
subplot(2,3,4);
scatter(y_train, final_pred, 50, 'filled', 'MarkerFaceAlpha', 0.6);
hold on;
...
% Save figure
saveas(gcf, fullfile(output_path, 'Random_Forest_Results.png'));
fprintf('Figure saved: %s\n', fullfile(output_path, 'Random_Forest_Results.png'));
...
save(fullfile(output_path, 'Random_Forest_Model.mat'), ...
     'final_model', 'best_params', 'best_validation_metrics', ...
     'final_r2', 'final_rmse', 'final_mae', 'oob_error_final', ...
     'vif_values', 'correlation_matrix', 'x_min', 'x_range', ...
     'results', 'raw_importance', 'relative_importance_rf', ...
     'nse', 'pbias', 'generalization_gap_r2', 'variable_names_cell', '-v7.3');

% Create summary report
report_file = fullfile(output_path, 'Random_Forest_Report.txt');
...
fprintf(fid, 'Cross-Validation Results (5-fold with 5 repeats):\n');
...
for i = 1:length(variable_names_cell)
    fprintf(fid, '  %s: %.4f\n', variable_names_cell{i}, relative_importance_rf(i));
end

fprintf(fid, '\nRaw Variable Importance (OOBPermutedPredictorDeltaError):\n');
for i = 1:length(variable_names_cell)
    fprintf(fid, '  %s: %.4f\n', variable_names_cell{i}, raw_importance(i));
end
